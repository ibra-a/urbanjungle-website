...
def get_favorites():
    ...
    try:
        user_id = request.current_user['user_id']
        favorites = auth_manager.get_user_favorites(user_id)
        return jsonify({'favorites': favorites}), 200
        
    except Exception as e:
        return jsonify({'error': f'Failed to get favorites: {str(e)}'}), 500

@app.route('/api/favorites/<product_id>', methods=['POST'])
@require_auth
def add_favorite(product_id):
    ...
    try:
        ...
        result = auth_manager.add_favorite(
            user_id=user_id,
        ...
        if result['success']:
            return jsonify(result), 201 if result['added'] else 200
        else:
            return jsonify(result), 400
...
def remove_favorite(product_id):
    ...
    try:
        ...
        result = auth_manager.remove_favorite(user_id, product_id)
