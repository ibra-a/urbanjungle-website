...
const Men = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // TODO: Update database to populate gender field, then use: getProductsByGender('MEN')
      const response = await apiService.getProducts();
      const allProductsData = response.products || [];
      
      // Filter for men's products by name/description if gender is null
      const menProducts = allProductsData.filter(product => {
        const name = (product.product_name || '').toLowerCase();
        const desc = (product.description || '').toLowerCase();
        const cat = (product.category || '').toLowerCase();
        
        // If gender is set and is MEN/MALE/UNISEX, include it
        if (product.gender) {
          const gender = product.gender.toUpperCase();
          return gender === 'MEN' || gender === 'MALE' || gender === 'UNISEX' || gender === 'M';
        }
        
        // Otherwise, show all products (since gender is mostly null)
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div className={`max-container padding transition-all duration-300 ${showFilters ? 'lg:ml-80' : ''}`}>
          <div className="text-black text-2xl mb-6">Loading Men's Collection...</div>
          <motion.div 
          ...
          >
            {[...Array(8)].map((_, i) => (
              <div key={i} className="nike-card rounded-2xl overflow-hidden">
                <LoadingSkeleton variant="card" className="h-64 bg-gray-100" />
                <div className="p-4 space-y-2">
                  <LoadingSkeleton variant="text" className="bg-gray-100" />
                  <LoadingSkeleton variant="title" className="bg-gray-100" />
                </div>
              </div>
            ))}
          </motion.div>
    ...
    <div className="min-h-screen bg-white text-black">
      ...
      <div className={`max-container padding transition-all duration-300 ${showFilters ? 'lg:ml-80' : ''}`}>
        ...
        <motion.div
          ...
          className="flex items-center justify-between text-sm text-black/60 mb-6"
        >
        ...
        {error && (
          ...
          >
            ...
            <div className="flex gap-4">
              ...
              <Link 
                to="/shop"
                className="bg-gray-100 text-black px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors text-sm"
              >
                View All Products
              </Link>
            </div>
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <button
              ...
              className="hidden lg:flex items-center gap-2 px-4 py-3 bg-gray-100 border border-gray-200 rounded-lg text-black hover:bg-gray-200 transition-colors"
            >
            ...
            <select
              ...
              className="px-4 py-3 bg-gray-100 border border-gray-200 rounded-lg text-black focus:border-nike-coral focus:outline-none"
            >
              <option value="featured" className="bg-white">Featured</option>
              <option value="newest" className="bg-white">Newest</option>
              <option value="bestseller" className="bg-white">Best Sellers</option>
        ...
        <motion.button
          ...
          className="lg:hidden w-full flex items-center justify-center gap-2 mb-6 px-4 py-3 bg-gray-100 border border-gray-200 rounded-lg text-black"
        >
          <Filter size={20} />
          <span>Filters</span>
          {getActiveFilterCount() > 0 && (
        ...
        {!loading && !error && filteredProducts.length === 0 && (
          ...
          >
            ...
            <p className="text-black/70 mb-6">
              Try adjusting your search or filters, or check that your ERPNext inventory has men's items
            </p>
            <div className="flex gap-4 justify-center">
              <button
                onClick={clearAllFilters}
                className="bg-gradient-to-r from-nike-coral to-orange-500 text-white px-6 py-3 rounded-lg font-semibold hover:shadow-lg transition-all duration-300"
              >
                Clear All Filters
              </button>
              <Link
                to="/shop"
                className="bg-gray-100 text-black px-6 py-3 rounded-lg font-semibold hover:bg-gray-200 transition-all duration-300"
              >
                View All Products
              </Link>
            </div>
